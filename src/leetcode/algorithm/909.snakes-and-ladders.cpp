...
class Solution {
public:
    pair<int, int> numToIndexs(int num, int size) {
        num -= 1;
        int row = num / size;
        int col = num % size;
        pair<int, int> rv;
    ...
    int snakesAndLadders(vector<vector<int>>& board) {
        ...
        while (!q.empty()) {
            ...
            for (int i = 0; i < size; ++i) {
                ...
                q.pop();
                visited[cur] = true;
                for (int j = 1; j <= 6 && cur+j <= board.size()*board.size(); ++j) {
                    auto index = numToIndexs(cur+j, board.size());
                    int next = cur+j;
                    if (board[index.first][index.second] != -1) {
                        next = board[index.first][index.second];
                    }
